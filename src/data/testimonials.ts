import { Testimonial } from '../types';

export const testimonials: Testimonial[] = [
  {
    id: 1,
    name: "Marina Silva",
    photo: "https://i.imgur.com/EpZiZ6p.jpeg",
    rating: 5,
    comment: "O melhor açaí da cidade! Sempre peço o combo de 700ml e nunca me decepciono. Atendimento rápido e qualidade excepcional."
  },
  {
    id: 2,
    name: "Ricardo Oliveira",
    photo: "https://images.pexels.com/photos/220453/pexels-photo-220453.jpeg?auto=compress&cs=tinysrgb&w=600",
    rating: 5,
    comment: "Sou fã do açaí zero açúcar. Incrível como conseguem manter o sabor mesmo sem adição de açúcar. Recomendo!"
  },
  {
    id: 3,
    name: "Juliana Costa",
    photo: "https://i.imgur.com/rWf6QrZ.jpeg",
    rating: 4,
    comment: "Adoro os complementos gratuitos no primeiro pedido. O açaí com Nutella e KitKat é simplesmente divino."
  },
  {
    id: 4,
    name: "Carlos Mendes",
    photo: "https://i.imgur.com/LSQsdF6.jpeg",
    rating: 5,
    comment: "A promoção 'Pague 1 e Leve 2' é imbatível. Sempre divido com minha esposa e economizamos bastante!"
  },
  {
    id: 5,
    name: "Fernanda Lima",
    photo: "https://i.imgur.com/dImJfhq.jpeg",
    rating: 5,
    comment: "Entrega sempre pontual e o açaí chega cremoso e gelado. Perfeito para dias quentes!"
  },
  {
    id: 6,
    name: "Anderson Nogueira",
    photo: "https://i.imgur.com/eMrVnsY.jpeg",
    rating: 4,
    comment: "Os complementos são muito frescos. Adoro adicionar granola e castanha de caju no meu açaí de 500ml."
  },
  {
    id: 7,
    name: "Patrícia Moreira",
    photo: "https://i.imgur.com/brCUcFm.jpeg",
    rating: 5,
    comment: "Descobri o Açaí Express há 2 meses e já virei cliente fiel. O sabor é autêntico e os preços são justos!"
  }
];

export const randomNames = [
  "João", "Maria", "Pedro", "Ana", "Carlos", "Fernanda",
  "Lucas", "Juliana", "Marcos", "Beatriz", "Rafael", "Camila",
  "Ricardo", "Mariana", "Felipe", "Gabriela", "Gustavo", "Carolina",
  "Diego", "Amanda", "Rodrigo", "Letícia", "Bruno", "Natália",
  "Leonardo", "Larissa", "Thiago", "Vanessa", "Eduardo", "Patrícia"
];